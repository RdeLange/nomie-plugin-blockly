...
<div class="color-picker-inner"  >
	<button bind:clientHeight={inputHeight} style="background-color:{themecolor};color:{themefont}" class="select-color" on:click={(e) => toggleDropdown(e)} class:fake-focus={ddActive}>
		<div style="display: flex;;color:{themefont}">
			<div style="background: {value};" class="color-block"></div>
			<div class="caret" class:top={top} style="margin-right: .2rem;caret-color:{themefont}"></div>
		</div>
	</button>
	<input style="background-color:{themecolor};color:{themefont}" type="text" bind:value>
	
</div>

{#if ddActive}
<div class:top={top} bind:clientHeight={ddHeight} class="values-dropdown" use:clickOutside on:click_outside={clickOutsideDropdown} style="background-color:{themecolor};color:{themefont}">
